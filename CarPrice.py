...
Engine_capacity = st.number_input("Cylindrée (Engine capacity en ccm)", min_value=0.0, step=0.1)
...
# Bouton pour exécuter la prédiction et la recommandation
if st.button("Prédire et recommander"):
    try:
        ...
        # Recommandation de voitures similaires
        ...
        if existing_data_filtered.empty:
            ...
            for idx, row in existing_processed.iterrows():
                score = calculate_similarity_score(input_processed.iloc[0], row, features_weights)
                price_diff = abs(existing_data_filtered.loc[idx, 'price'] - predicted_price)
                price_penalty = min(price_diff / predicted_price, 1)
            ...
            for idx, score in similarity_scores[:5]:
                ...
                st.markdown(
                    ...
                    Moteur: {car['Engine_capacity']}ccm, {car['Power']}ch | {car['Fuel']} | {car['Transmission']}  
                    Similarité: {score:.2%}
                    """
                )
                st.markdown("---")
